...
interface TicketState {
  ...
  calculateKPIs: (role: Role, userId?: string, team?: Team) => void;
}
...
export const useTicketStore = create<TicketState>((set, get) => ({
  ...
  addTicket: async (ticketData) => {
    ...
    const preId = session.user?.id;
    const team = session.user?.team;

    if (!preId || !team) return;
  ...
  calculateKPIs: (role, userId, team) => {
    const { tickets } = get();
    let filteredTickets: Ticket[] = [];

    if (role === Role.PRE && userId) {
        filteredTickets = tickets.filter(t => t.preId === userId);
    } else if ((role === Role.ASM || role === Role.BDM) && team) {
        filteredTickets = tickets.filter(t => t.team === team);
    } else if (role === Role.ADMIN || role === Role.DATACR) {
